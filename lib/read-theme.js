    ...
    return Promise.map(themeFilesContent, function (themeFile) {
        return fs.readFile(path.join(theme.path, themeFile.file), 'utf8').then(function (content) {
            ...
            if (handlebarsMatch) {
                ...
                processQueries(theme, themeFile);
            }
        });
    }).then(function () {
        return theme;
...
const processQueries = function (theme, themeFile) {
    linter.verify({
        parsed: themeFile.parsed,
        rules: [
            require('./ast-linter/rules/mark-used-queries')
        ],
        source: themeFile.content,
        moduleId: themeFile.file
    });
    for (const query of linter.queries) {
        theme.queries.push(query);
    }
};
